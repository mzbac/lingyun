    ...
    additions.push(
      wrapSystemReminder(
        options.allowExternalPaths
          ? 'External paths are enabled (lingyun.security.allowExternalPaths=true). Tools like list/read can access paths outside the workspace.'
          : 'External paths are disabled (lingyun.security.allowExternalPaths=false). Tools must stay within the current workspace.',
      ),
    );
  }
