...
export function useEditorActions(
  ...
  const setLastSavedLayout = useCallback((layout: OfficeLayout) => {
    // Cancel any pending debounced save so we don't re-save a stale layout after an external reload/reset.
  ...
  const saveLayout = useCallback((layout: OfficeLayout) => {
    ...
    saveTimerRef.current = setTimeout(() => {
      const message: WebviewToExtensionMessage = { type: 'saveLayout', layout }
