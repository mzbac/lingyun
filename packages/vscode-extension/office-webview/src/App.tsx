...
import type { WebviewToExtensionMessage } from '../../src/shared/officeProtocol.js'
...
function App() {
  ...
  const { agents, selectedAgent, agentTools, agentStatuses, subagentTools, subagentCharacters, layoutReady } = useExtensionMessages(getOfficeState, editor.setLastSavedLayout, isEditDirty)
  ...
  const handleSelectAgent = useCallback((id: number) => {
    const message: WebviewToExtensionMessage = { type: 'focusAgent', id }
  ...
  const handleCloseAgent = useCallback((id: number) => {
    const message: WebviewToExtensionMessage = { type: 'closeAgent', id }
    vscode.postMessage(message)
  }, [])
  ...
  const handleClick = useCallback((agentId: number) => {
    ...
    const message: WebviewToExtensionMessage = { type: 'focusAgent', id: focusId }
    vscode.postMessage(message)
  }, [])
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden' }}>
      <style>{`
        @keyframes lingyun-pulse {
          0%, 100% { opacity: 1; }
          50% { opacity: 0.3; }
        }
        .lingyun-pulse { animation: lingyun-pulse ${PULSE_ANIMATION_DURATION_SEC}s ease-in-out infinite; }
      `}</style>
